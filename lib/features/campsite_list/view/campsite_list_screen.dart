...
import '../widget/campsite_search_field.dart';
import '../widget/featured_section.dart';
import '../widget/campsite_filter_section.dart';
...
class CampsiteListScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CampsiteListScreen> createState() =>
      _CampsiteListScreenState();
}
...
class _CampsiteListScreenState extends ConsumerState<CampsiteListScreen> {
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            ...
            Text(StringConstants.appTitle,
                style:
                theme.textTheme.titleMedium?.copyWith(color: AppColors.secondaryDark)),
          ],
        ),
      ),
      body: async.when(
        data: (_) => Padding(
          ...
          child: CustomScrollView(
            ...
            slivers: [
              SliverToBoxAdapter(
                  child: CampsiteSearchField(controller: _searchCtrl)),
              const SliverToBoxAdapter(child: FeaturedSection()),
              SliverToBoxAdapter(
                  child:
                  CampsiteFilterSection(filter: filter, controller: _searchCtrl)),
              SliverToBoxAdapter(child: _buildHeader(theme)),
              if (filtered.isEmpty)
                SliverFillRemaining(
                  child: Center(
                      child: Text(StringConstants.noCampsitesFound,
                ...
                SliverList(
                  delegate: SliverChildBuilderDelegate(
                        (_, idx) =>
                        CampsiteCard(campsite: filtered[idx]),
        ...
        error: (e, _) => Center(
            child: Text(StringConstants.errorLoading,
                style: theme.textTheme.bodyLarge
  ...
  Widget _buildHeader(ThemeData theme) {
    return Padding(
      ...
      child: Text(StringConstants.campingSites,
